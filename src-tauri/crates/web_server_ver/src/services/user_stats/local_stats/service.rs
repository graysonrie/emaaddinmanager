use crate::services::addins_registry::{AddinModel, AsyncAddinsRegistryServiceType};
use crate::services::db::service::AppDbService;
use crate::services::user_stats::db::LocalStatsDbHandler;
use crate::services::user_stats::local_stats::models::RefreshUserStatsModel;
...
impl UserStatsService {
    ...
    pub async fn refresh_user_stats(
        &self,
        user_email:&str,
        model: RefreshUserStatsModel,
    ) -> Result<Option<UserStatsModel>, String> {
        let disciplines = model.disciplines;
        let installed_addins = model.installed_addins;

        let published_addins = self.get_published_addins(&user_email).await?;
        ...
        let user_stats = table.get_user(user_email.to_string()).await?;
    ...
    async fn get_published_addins(
        &self,
        user_email: &str,
    ) -> Result<Vec<PublishedAddinModel>, String> {
        let all_addins = self
            .addins_registry
            .get_addins()
            .await
